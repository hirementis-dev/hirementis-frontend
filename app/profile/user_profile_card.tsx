            ...
            {editingBio ? (
              <div className="flex flex-col gap-2">
                ...
                <div className="flex gap-2">
                  ...
                  <Button size="sm" variant="outline" onClick={() => { setEditingBio(false); setBio(user.bio || ''); }}>Cancel</Button>
                </div>
              </div>
            ) : (
              <div className="flex items-center gap-2">
                <p className="mt-1">{user.bio || <span className="text-gray-400">No bio added.</span>}</p>
          ...
          <div>
            ...
            <div className="mt-1 grid grid-cols-2 gap-2">
              <div className="p-2 bg-gray-50 rounded-md">
                <div className="text-lg font-semibold">{user.interviews?.length || 0}</div>
                <div className="text-xs text-gray-500">Interviews</div>
              </div>
              <div className="p-2 bg-gray-50 rounded-md">
                <div className="text-lg font-semibold">
                  {user.interviews && user.interviews.length > 0
                    ? Math.round(user.interviews.reduce((acc, interview) => acc + (interview.score || 0), 0) / user.interviews.length)
                    : 0}
                  %
                </div>
